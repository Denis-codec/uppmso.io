...
if($_GET['mode']=='0'){
	...
	for ($j=0; $j< count($ff); $j++) { 
		print('<a href="cmsland.php?mode=3&j='.$j.'" class="mytext">'.$ff[$j].'</a>');
	};
};
...
if($_GET['mode']=='4'){
	...
	print "<br><br><center>Текст был успешно изменен.<p><a href='cmsland.php?mode=0'>Вернуться к списку текстов</a><p>ПРИ ПРОСМОТРЕ ИЗМЕНЕНИЙ НА САЙТЕ НЕ ЗАБУДЬТЕ ОБНОВИТЬ ЕГО СТРАНИЦУ В БРАУЗЕРЕ";
};
...
// Форма для HTML кода
if($_GET['mode']=='5'){
	$Body=htmlspecialchars(file_get_contents($pagename));
